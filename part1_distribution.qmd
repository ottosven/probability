...
$\lim_{\varepsilon \to 0, \varepsilon > 0} F(a - \varepsilon)$ denotes the left limit at $a$ while $\lim_{\varepsilon \to 0, \varepsilon > 0} F(a+\varepsilon)$ denotes the right limit at $a$. When approaching from the left, the CDF can have a jump, while when approaching from the right, the CDF cannot jump (due to right-continuity).
...
For discrete random variables, independence can be expressed using PMFs: $Y$ and $Z$ are independent if and only if $\pi_{Y|Z=b}(a) = \pi_Y(a)$ for all $a,b$. Similarly, for continuous random variables, independence means the conditional PDF factorizes $f_{Y|Z=b}(a) = f_Y(a)$.
