...
text(trans3d(-12,10,0,pmat),"Experience",cex=.75)
dev.off()
...
plot(y,f[,1],type="l",lty=1,xlim=c(-0.1,50),ylim=c(0,.14),xaxs="i",yaxs="i",ylab="",xlab="",bty="n",xaxt="n")
lines(y,f[,2])
lines(y,f[,3])
...
## ####################
## Conditional CDF wage experience
## ####################
png("./img/sec03-WageExCondCDF.png", width = 480, height = 300)
par(mar = c(2, 2, 0.9, 0.5), cex=1.6, lwd = 2, cex.axis=0.9)
wage = statisticsdata::allbus21$wage
ex = statisticsdata::allbus21$experience
n6 <- length(wage/100)^(-1/6)
...
plot(y,cumsum(f[,1])/sum(f[,1]),type="l",lty=1,xlim=c(-0.1,50),ylim=c(0,1),xaxs="i",yaxs="i",ylab="",xlab="",bty="n",xaxt="n")
# lines(y,cumsum(f[,2])/sum(f[,2]))
lines(y,cumsum(f[,3])/sum(f[,3]))
# lines(c(0,0),c(0,f[1,1]))
axis(side=1,seq(-10,70,10),cex.axis=.75)
text(15,.923,"F(a|b=0)",cex=.75)
# text(21,.1,"f(a|b=10)",cex=.75)
text(18,.44,"F(a|b=25)",cex=.75)
dev.off()
